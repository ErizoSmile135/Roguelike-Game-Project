        ...
        */
        ...
        onMovePath(payload) {
          ...
          const pos = this.em.getComponent(entity, _crd && Position === void 0 ? (_reportPossibleCrUseOfPosition({
            error: Error()
          }), Position) : Position); // Если визуала нет или путь пуст — ничего не делаем

          if (!(viewComp != null && viewComp.node) || path.length === 0) {
            return;
          }

          const node = viewComp.node; // Сохраняем Z, чтобы не уронить глубину слоя

          const z = node.getPosition().z; // Начинаем сборку цепочки анимаций
          ...
          for (const step of path) {
            ...
            pos.x = step.x;
            pos.y = step.y; // Добавляем в цепочку tween, который за stepDuration секунд
...
//# sourceMappingURL=ededb486f009c03da5fff30d69dcdb1ed03ab380.js.map